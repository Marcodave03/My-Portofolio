...
import { AlignLeft, ScrollText, Github, ChevronRight, Sun, Moon } from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, setIsOpen, onProjectClick, isDark, toggleTheme }) => {
  const [mobileOpen, setMobileOpen] = useState(false);


  const projects = [
    {
      name: 'Disaster Tweets Classifier',
      description: 'NLP model to identify real disaster tweets',
      tech: ['Python', 'BERT', 'Deep Learning'],
      query: "Tell me about your Disaster Tweets project",
    },
    {
      name: 'Portfolio Chatbot',
      description: 'AI-powered portfolio website',
      tech: ['React', 'Javascript', 'Python'],
      query: "Tell me about your portfolio website",
    },
    {
      name: 'Jakarta 2024 Governor Election Tweets Analysis',
      description: 'Simple yet insightful Exploratory Data Analysis',
      tech: ['Python', 'EDA', 'Wordcloud'],
      query: "Tell me about your governor election tweet analysis",
    },
  ];
    ...
    <div>
      ...
      <div className="fixed top-4 left-4 z-20 md:hidden">
        <button onClick={() => setMobileOpen(!mobileOpen)} className="p-2">
          <AlignLeft className={`w-6 h-6 ${isDark ? 'text-white' : 'text-gray-700'}`} />
        </button>
      </div>
      ...
      >
        <button
          ...
          className={`absolute -right-3 top-4 w-6 h-6 border rounded-full flex items-center justify-center
            ${isDark ? 'bg-black border-[#252525] hover:bg-[#252525]' : 'bg-white border-gray-200 hover:bg-gray-50'}`}
        >
          <ChevronRight
              ...
              <div className={`divide-y ${isDark ? 'divide-[#252525]' : 'divide-gray-100'}`}>
                {projects.map((project) => (
                  <div key={project.name} className="py-2 first:pt-0 last:pb-0">
                    ...
                    >
                      <p className={`text-xs md:text-sm font-medium mb-1 ${isDark ? 'text-white' : 'text-gray-900'}`}>
                        {project.name}
                      </p>
                      <p className={`text-xs mb-1.5 ${isDark ? 'text-[#A1A1A1]' : 'text-gray-500'}`}>
                        {project.description}
                      </p>
                        ...
                        {project.tech.map((tech) => (
                          <span
                            ...
                            className={`px-2 py-0.5 text-xs rounded-full
                              ${isDark ? 'bg-[#252525] text-[#A1A1A1]' : 'bg-gray-100 text-gray-600'}`}
                          >
                            {tech}
                          </span>
                        ))}
      ...
      {/* Mobile Sidebar */}
      <div
        className={`fixed top-0 left-0 h-full transition-transform duration-300 ease-in-out z-30
      ...
      >
        <div className={`p-3 border-b flex items-center justify-between ${isDark ? 'border-[#252525]' : 'border-gray-200'}`}>
          <div className="flex items-center">
            <img src="/9edcdf8d-d69d-402d-9c0b-e43646659ea6.jpg" alt="Profile" className="w-8 h-8 rounded-full object-cover" />
            <div className="ml-3">
              <h2 className={`font-semibold text-sm ${isDark ? 'text-white' : 'text-gray-900'}`}>Ferdinand</h2>
              <p className={`text-xs ${isDark ? 'text-[#A1A1A1]' : 'text-gray-500'}`}>Data Scientist</p>
            </div>
          </div>
          <button 
            onClick={toggleTheme} 
